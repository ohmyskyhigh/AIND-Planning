...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        self.a_levels.append(set()) #create a set element in a_levels
        s_prelevel = self.s_levels[level] #get the previous level
        for action in self.all_actions: # each action in all available actions
            node_a = PgNode_a(action) #action in node form
            prenode = node_a.prenodes
            if prenode.issubset(s_prelevel): #if all the precondition of node are available in previous level
                for node_s in s_prelevel:
                    if node_s in prenode:
                        node_s.children.add(node_a) #link action to literal's children node
                        node_a.parents.add(node_s) #link literal to action's parent's node
    ...
    def add_literal_level(self, level):
        ...
        for action in self.a_levels[level-1]: #in every action
            for effect in action.effnodes: #in every effect of an action
                action.children.add(effect) #add effect to action's children
                effect.parents.add(action) #add action to effect's parents
                self.s_levels[level].add(effect) #effects as the next layer
    ...
    def h_levelsum(self) -> int:
        ...
        """
        level_sum = 0
        goals = self.problem.goal #get the goals
        s_levels = self.s_levels #get the level of states
        for goal in goals: #for each goal (subgoal independence)
            node = PgNode_s(goal, True) #(goal in PgNode_s form)
            s_levels_list = enumerate(s_levels)
            for level, s_nodes in s_levels_list:
                if node in s_nodes:
                    level_sum += level #add the level as level cost and sum up
                    break #stop the loop and move on to another goal
        return level_sum
